...
class comunicationHandler:
    ...
    def __init__(self, server_address, consumer_group_name):
        ...
        try:
            ...
            raise (
                f"An error occured while the AdminClient tried to connect to Kafka. Error: {str(e)}"
            )
        ...
        try:
            ...
            raise (
                f"An error occured while the Consumer tried to connect to Kafka. Error: {str(e)}"
            )
        ...
        try:
            ...
            raise (
                f"An error occured while the Producer tried to connect to Kafka. Error: {str(e)}"
            )
    ...
    def create_topic(self, topic_name, num_partitions, replication_factor):
        ...
        for future in futures:
            try:
                future.result()
                print(
                    f"Topic {future.topic} created with replication factor {future.replication_factor}"
                )
    ...
    def set_consumer_topic_subscribtion(
        self, topic, auto_topic_creation=True, num_partitions=1, replication_factor=1
    ):
        """
        ...
        try:
            ...
            if "Unknown topic or partition" in str(e):
                if auto_topic_creation:
                    ...
                    raise (
                        f"Topic {topic} does not exist. auto_topic_creation set to false."
                    )
            else:
