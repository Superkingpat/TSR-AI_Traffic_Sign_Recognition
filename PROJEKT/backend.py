...
def send_data_to_flask(packet):
    url = f"http://127.0.0.1:5000/receive_data"
...
class CameraManager:
    ...
    def start_gps_tracking(self):
        def run_gps():
            ...
            self.gps_controller.run_continuous()
    ...
    def get_camera(self) -> cv2.VideoCapture:
        if self.camera is None:
            self.camera = cv2.VideoCapture(1)  # Spremeni na 1
...
class KafkaTrafficSignConsumer:
    ...
    def process_message(self, msg):
        try:
            ...
            self.current_signs = [
                self.reverse_traffic_signs.get((r.strip()), "unknown") for r in results
            ]

            send_data_to_flask(self.current_signs)
...
def capture_frames():
    while camera_manager.is_capturing:
        try:
            ...
            if success:
                swapped_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB_SWAP)
