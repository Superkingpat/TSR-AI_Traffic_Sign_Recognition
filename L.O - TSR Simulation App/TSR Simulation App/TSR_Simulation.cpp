...
void TSR_Simulation::InitRenderObjects() {
    ...
    m_objectHandler.loadOBJ("car", "Models/car.obj", ObjectType::CAR); // ObejctType:: Car - prepusca premikanje
    ...
    wd.Position = glm::vec3(0.120f, 0.15f, 0.3f); // wd.Position = glm::vec3(0.1f, 0.15f, 0.3f);
    wd.Rotation.y = 0.f;
...
void TSR_Simulation::Draw() {
    ...
    if (ImGui::BeginMainMenuBar()) {
        if (ImGui::BeginMenu("View")) {
            ImGui::MenuItem("Camera Controls", NULL, &m_showCameraControls);
            ImGui::MenuItem("Vehicle", NULL, &m_showVehicle);
            ImGui::MenuItem("Performance", NULL, &m_showPerformance);
            ImGui::EndMenu();
        }
        if (ImGui::BeginMenu("Help")) {
            if (ImGui::MenuItem("Controls Guide")) {
                m_showControlsGuide = true;
            }
            ImGui::EndMenu();
        }
...
// Add this helper function to your class to verify object state
void TSR_Simulation::VerifyObjectState(const std::string& objName) {
    for (const auto& obj : m_objectHandler.getObjectsVector()) {
        if (objName == "grass" && obj->geometry->waterApplied) {
            std::cout << "\n=== Grass Object State ===" << std::endl;
            std::cout << "Water Applied: " << obj->geometry->waterApplied << std::endl;
            std::cout << "Number of textures: " << obj->geometry->textures.size() << std::endl;
            std::cout << "VAO: " << obj->geometry->VAO << std::endl;
            std::cout << "Number of indices: " << obj->geometry->numOfIndecies.size() << std::endl;
            for (size_t i = 0; i < obj->geometry->textures.size(); i++) {
                std::cout << "Texture " << i << " ID: " << obj->geometry->textures[i].texture << std::endl;
                std::cout << "Texture " << i << " used: " << obj->geometry->textures[i].used << std::endl;
            }
        }
    }
}
